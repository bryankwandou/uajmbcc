    ...
    <MarsPlayerProvider tracks={tracks}>
      <main className="bg-field relative min-h-screen">
        ...
        <div className="mx-auto max-w-5xl space-y-16 px-5 pb-24">
          {tracks.map((t) => (
            <TrackSection key={t.slug} track={t} />
          ))}
          <Docs />
          <Sibling />
        </div>
    ...
    <header className="sticky top-0 z-30 border-b border-[color:var(--line)] bg-[color:var(--bg)]/85 backdrop-blur-xl">
      <nav className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-5 py-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Link
            href="/"
            className="rounded border border-[color:var(--line)] px-3 py-1.5 text-xs text-[color:var(--muted)] duration-200 ease-crisp [transition-property:opacity] hover:border-[color:var(--line-strong)] hover:text-[color:var(--text)]"
          >
            {d.nav.home}
          </Link>
        </div>
    ...
    <section className="border-b border-[color:var(--line)]">
      <div className="mx-auto max-w-5xl px-5 py-20 md:py-28">
        <Reveal>
          ...
          <h1 className="mt-5 font-display text-4xl font-semibold leading-[1.05] tracking-tight text-[color:var(--text)] md:text-6xl">
            {d.hero.title} <span className="text-[color:var(--accent)]">{d.hero.titleEm}</span>
          </h1>
...
function TrackSection({ track }: { track: MarsTrack }) {
  const { locale } = useApp();
  const d = marsDict(locale);
  return (
      ...
      <Reveal>
        <div className="flex items-baseline gap-4">
          <div className="min-w-0">
            <h2 className="font-display text-2xl font-semibold tracking-tight text-[color:var(--text)] md:text-3xl">
              {track.title}
            </h2>
          ...
          <div className="panel h-full p-7">
            ...
            <div className="mt-5 space-y-5">
              {track.lyrics.map((s, i) => (
                <div key={i}>
                  <div className="text-[10px] uppercase tracking-[0.22em] text-[color:var(--accent)]">{s.label}</div>
...
function VersionRow({ version, track }: { version: MarsVersion; track: MarsTrack }) {
  ...
  return (
    <div className="panel overflow-hidden">
      ...
      <div
        className={`flex flex-wrap items-center gap-x-4 gap-y-3 border-t border-[color:var(--line)] p-4 duration-200 ease-crisp [transition-property:border-color] ${
      ...
      >
        ...
        <div className="min-w-[14rem] flex-1">
          <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
            <h4 className="font-display text-sm font-semibold text-[color:var(--text)]">
  ...
  return (
    <section id="dokumentasi" className="scroll-mt-20 pt-16">
      <Reveal>
        ...
        <h2 className="mt-4 font-display text-2xl font-semibold tracking-tight text-[color:var(--text)] md:text-3xl">
          {d.docs.title}
        </h2>
      </Reveal>
          ...
          <div className="panel h-full p-7">
            <dl className="space-y-3.5">
              {[
                ...
                <div key={k} className="flex flex-wrap justify-between gap-2 border-b border-[color:var(--line)] pb-2.5 last:border-0">
...
function Sibling() {
  ...
  return (
    <section className="pt-4">
      <Reveal>
        <div className="panel-feature p-7">
          <h2 className="font-display text-lg font-semibold text-[color:var(--text)]">{d.sibling.title}</h2>
          <p className="mt-2.5 max-w-2xl text-sm leading-relaxed text-[color:var(--muted)]">{d.sibling.body}</p>
          <a
            href={d.sibling.href}
            target="_blank"
            rel="noopener noreferrer"
            className="btn-ghost mt-5 inline-block px-4 py-2 text-[13px]"
          >
            {d.sibling.cta}
          </a>
        </div>
