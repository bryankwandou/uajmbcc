/* Certificate registry for UAJM Blockchain Club.
 *
...
/* ── identity matching ─────────────────────────────────────────────────────
   ...
   identity behind them must. The name is the whole key here, so the
   comparison has to be forgiving about how it was typed. */
...
/* The salt is public, and it is meant to be: it stops one registry from being
   ...
   honestly do. */
const KEY_SALT = "uajm-bcc/sertifikat/v1";

export async function identityKey(fullName: string): Promise<string> {
  const subtle = globalThis.crypto?.subtle;
  const material = `${KEY_SALT}|${normName(fullName)}`;
...
export function matches(rec: CertRecord, fullName: string): boolean {
  return normName(rec.fullName) === normName(fullName);
}
...
export async function findFor(records: CertRecord[], fullName: string): Promise<CertRecord[]> {
  const key = await identityKey(fullName);
  const name = fullName.trim().replace(/\s+/g, " ");
  const hits: CertRecord[] = [];
  for (const rec of records) {
    if (rec.key && rec.key === key) hits.push({ ...rec, fullName: name });
    else if (rec.fullName && matches(rec, fullName)) hits.push(rec);
  }
  return hits;
}
...
export async function lookup(fullName: string): Promise<CertRecord[]> {
  ...
  return (rows as (CertRecord & { k16?: string })[])
    .filter((r) => r.k16 === stub)
    .map((r) => ({ ...r, id: `${r.k16}-${r.createdAt}`, key, fullName: name }));
}
...
export async function login(user: string, pass: string): Promise<Session | null> {
  ...
  // Terkunci setelah terlalu banyak percobaan, atau server bermasalah.
...
export async function saveRecord(
  ...
  const key = await identityKey(rec.fullName);
