 ...
 */
const SESSION_KEY = "uajmbcc.cert.session";
...
export function AdminPanel({
  ...
  return session ? (
    <Workspace
      ...
      onSession={signIn}
      onSignOut={signOut}
      siteName="UAJM Blockchain Club"
      registerFallback={links.whatsapp}
      brand={{ name: "UAJM Blockchain Club", handle: "uajm_bcc", avatar: <LogoMark size={40} /> }}
      cert={(perms) => (
...
function SignIn({
  ...
  return (
    <form onSubmit={submit} className="panel-feature mx-auto max-w-md p-7">
...
function Dashboard({
  ...
  /* One paste fills the roster: one name per line, which is what a copy out
     of the attendance sheet already looks like. */
    ...
    <div>
      ...
      <div className="mt-8 grid items-start gap-6 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        <form
          ref={formRef}
          onSubmit={save}
          className="panel h-fit p-6 lg:sticky lg:top-6 lg:max-h-[calc(100vh-3rem)] lg:overflow-y-auto"
        >
                ...
                >
                  ...
                  <div className="flex items-center gap-4 sm:justify-end">
                    ...
                    <button
                      type="button"
                      onClick={() => remove(r)}
                      className="link-quiet border-l border-[color:var(--line)] pl-4 text-xs text-[color:var(--accent)]"
                    >
                      {d.admin.del}
                    </button>
                    )}
...
const inputCls =
  "w-full rounded border border-[color:var(--line)] bg-[color:var(--surface-2)] px-3 py-2.5 text-sm text-[color:var(--text)] outline-none placeholder:text-[color:var(--faint)] hover:border-[color:var(--line-strong)] focus-visible:border-[color:var(--accent)]";
