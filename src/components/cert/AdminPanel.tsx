...
export const ACCOUNTS: Account[] = [
  ...
  { user: "sekretaris.uajmbcc", pass: "sekretaris.uajmbcc", role: "sekretaris" },
  { user: "pembina.uajmbcc", pass: "pembina.uajmbcc", role: "pembina" },
];

const SESSION_KEY = "uajmbcc.cert.admin";
const ROLE_KEY = "uajmbcc.cert.role";
...
function SignIn({
  ...
  return (
    <form
      ...
      className="panel-feature mx-auto max-w-md p-7"
    >
      <div className="text-[10px] uppercase tracking-[0.25em] text-[color:var(--accent)]">{d.admin.signIn}</div>
      ...
      {error && (
        <p role="alert" className="mt-3 text-xs text-[color:var(--accent)]">
          {error}
        </p>
      )}
      ...
      <p className="mt-5 border-t border-[color:var(--line)] pt-4 text-[11px] leading-relaxed text-[color:var(--faint)]">
        {d.admin.note}
      </p>
    </form>
  );
...
function Dashboard({
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      const rec: CertRecord = {
        ...
        title: draft.title.trim() || "Sertifikat UAJM Blockchain Club",
  ...
  /* One paste fills the roster: one name per line, which is what a copy out of
     the attendance sheet already looks like. */
    ...
    try {
      ...
      for (const line of lines) {
        ...
        await putRecord({
          id: newId(),
          fullName: name,
          title: "Sertifikat UAJM Blockchain Club",
          event: "",
          issuedAt: "",
          fileName: "",
          mime: "",
  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="text-[10px] uppercase tracking-[0.25em] text-[color:var(--accent)]">{d.admin.title}</div>
          <h2 className="mt-2 font-display text-2xl font-bold uppercase tracking-tight text-[color:var(--text)]">
        ...
        {Array.from({ length: Math.max(SLOTS, filled) }, (_, i) => (
          <span
            ...
            className={`h-2.5 w-6 rounded-sm ${
              i < filled
                ? "bg-gradient-to-r bg-[color:var(--accent)]"
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        <form onSubmit={save} className="panel h-fit p-6">
          ...
          <div className="mt-5 space-y-4">
            ...
            <Field label={d.admin.fFile}>
              <input
                ...
                className="w-full text-xs text-[color:var(--muted)] file:mr-3 file:cursor-pointer file:border file:border-[color:var(--line)] file:bg-[color:var(--surface-2)] file:px-3 file:py-2 file:text-xs file:text-[color:var(--text)]"
              />
          ...
          {error && (
            <p role="alert" className="mt-4 text-xs text-[color:var(--accent)]">
              {error}
            </p>
          )}
        ...
        <div className="panel p-6">
          ...
          <div className="mt-4 border-b border-[color:var(--line)] pb-5">
              ...
              {records.map((r) => (
                <li
                  ...
                  className={`flex flex-wrap items-start justify-between gap-3 px-2 py-3.5 ${
                    dragId === r.id
                      ? "bg-[color:var(--surface)] outline-dashed outline-1 outline-[color:var(--accent)]"
                      : ""
                  }`}
                >
                  <div className="min-w-0">
                    ...
                    {!hasFile(r) && (
                      <div className="mt-1.5 text-[11px] text-[color:var(--accent)]">{d.admin.dropHere}</div>
                    )}
                  </div>
                  <div className="flex shrink-0 items-center gap-3">
                    <span className="panel px-2 py-0.5 font-mono text-[10px] text-[color:var(--faint)]">
                      {hasFile(r)
                        ? r.source === "published"
                          ? d.admin.published
                          : d.admin.local
                    ...
                    <button
                      type="button"
                      onClick={() => remove(r)}
                      className="link-quiet text-xs text-[color:var(--accent)]"
                    >
                      {d.admin.del}
                    </button>
                  </div>
          ...
          <p className="mt-6 border-t border-[color:var(--line)] pt-4 text-[11px] leading-relaxed text-[color:var(--faint)]">
            {d.admin.publishHelp}
          </p>
        </div>
      </div>
...
const inputCls =
  "w-full rounded border border-[color:var(--line)] bg-[color:var(--surface-2)] px-3 py-2.5 text-sm text-[color:var(--text)] outline-none duration-200 ease-crisp [transition-property:opacity] placeholder:text-[color:var(--faint)] hover:border-[color:var(--line-strong)] focus-visible:border-[color:var(--accent)]";
...
function ToolButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="border border-[color:var(--line)] px-3.5 py-2 text-xs text-[color:var(--muted)] duration-200 ease-crisp [transition-property:opacity] hover:border-[color:var(--line-strong)] hover:text-[color:var(--text)] disabled:opacity-60"
    >
      {children}
    </button>
  );
