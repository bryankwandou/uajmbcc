  ...
  return (
    <article className="panel overflow-hidden">
      <div className="grid gap-0 md:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)]">
        <div className="relative min-h-[220px] border-b border-[color:var(--line)] bg-[color:var(--surface-2)] p-4 md:border-b-0 md:border-r">
          {preview ? (
            ...
            <div
              ...
              className="h-full min-h-[200px] w-full opacity-60 rounded bg-[color:var(--surface)]"
            />
          )}
        </div>

        <div className="p-6">
          <h3 className="font-display text-lg font-bold uppercase leading-tight tracking-tight text-[color:var(--text)]">
            {rec.title}
          </h3>
          <p className="mt-1.5 text-sm leading-relaxed text-[color:var(--muted)]">{rec.event}</p>

          <dl className="mt-5 space-y-2 border-t border-[color:var(--line)] pt-4 text-xs">
          ...
          {formats.length > 0 && (
            <>
              <div className="mt-6 text-[10px] uppercase tracking-[0.2em] text-[color:var(--accent)]">
                {d.res.formats}
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
                {formats.map((f, i) => (
                  <button
                    ...
                    className={
                      i === 0
                        ? "btn-primary px-4 py-2 text-xs disabled:opacity-60"
                        : "border border-[color:var(--line)] px-4 py-2 text-xs font-semibold text-[color:var(--text)] duration-200 ease-crisp [transition-property:opacity] hover:border-[color:var(--line-strong)] disabled:opacity-60"
                    }
                  >
          ...
          <button
            ...
            disabled={busy !== null}
            className="link-quiet mt-4 inline-block text-xs text-[color:var(--muted)] underline decoration-[color:var(--line-strong)] underline-offset-4 hover:text-[color:var(--text)] disabled:opacity-60"
          >
