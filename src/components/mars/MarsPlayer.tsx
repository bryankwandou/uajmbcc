...
export function MarsPlayerProvider({ tracks, children }: { tracks: MarsTrack[]; children: ReactNode }) {
  ...
  return (
    <PlayerCtx.Provider value={ctx}>
      ...
      {loaded && (
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-[color:var(--line)] bg-[color:var(--bg)]/95 backdrop-blur-xl">
          <div className="mx-auto max-w-6xl px-4 pb-3 pt-2.5 sm:px-5">
            ...
            >
              <div className="relative h-1.5 rounded-full bg-[color:var(--surface-2)]">
                {duration > 0 &&
                  ranges.map(([s, e], i) => (
                    <div
                      key={i}
                      className="absolute inset-y-0 rounded-full bg-[color:var(--line-strong)]"
                      style={{ left: `${(s / duration) * 100}%`, width: `${((e - s) / duration) * 100}%` }}
                    />
                  ))}
                <div
                  className="absolute inset-y-0 left-0 rounded-full bg-[color:var(--accent-solid)]"
                  style={{ width: `${pct}%` }}
                />
                <div
                  className="absolute top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[color:var(--accent)] opacity-0 duration-200 ease-crisp [transition-property:opacity] group-hover:opacity-100 group-focus-visible:opacity-100"
                  style={{ left: `${pct}%` }}
                />
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="truncate text-[11px] text-[color:var(--faint)]">
                  ...
                  {waiting && <span className="text-[color:var(--accent)]"> · memuat…</span>}
                  {error && <span className="text-[color:var(--accent)]"> · {error}</span>}
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className={`rounded border px-2 py-1 font-mono text-[10px] uppercase duration-200 ease-crisp [transition-property:opacity] ${
                      loaded.format === f
                        ...
                        : "border-[color:var(--line)] text-[color:var(--faint)] hover:text-[color:var(--text)]"
                    }`}
                  >
                    {f}
                  </button>
                ...
                <button
                  ...
                  className="grid h-8 w-8 place-items-center rounded border border-[color:var(--line)] text-[color:var(--faint)] hover:text-[color:var(--text)]"
                >
...
function IconButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      aria-pressed={active}
      className={`grid h-8 w-8 place-items-center rounded border border-[color:var(--line)] duration-200 ease-crisp [transition-property:opacity] hover:text-[color:var(--text)] ${
        active ? "text-[color:var(--accent)]" : "text-[color:var(--faint)]"
      }`}
    >
